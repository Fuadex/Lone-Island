...
lrc_file = open('PinkFloyd2.lrc.txt') #.lrc.txt file to be read
lrc_string = ''.join(lrc_file.readlines())
lrc_file.close()
subs = pylrc.parse(lrc_string) #all the magical parsing works here (pylrc)

def most_similar(word):
  queries = [s for s in word.vocab if s.is_lower == word.is_lower and s.prob <=-9.5] #Here you may define the similarity cut-off
  by_similarity = sorted(queries, key=lambda s: word.similarity(s), reverse=True)
  return by_similarity[:20] #You can manipulate by how many similar words the code will return
...
with open('Pinker lol.csv', 'wb') as csvfile: #Please define a specific file output
